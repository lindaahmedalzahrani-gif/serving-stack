...
if ! docker run -d --name "$NAME" -p "${PORT}:8000" -e API_KEY="$API_KEY" -v hf-cache:/home/app/.cache/huggingface "$IMAGE" >/dev/null 2>&1; then
...
resp=$(curl -s "http://localhost:${PORT}/v1/chat/completions" -H "Content-Type: application/json" -H "Authorization: Bearer $API_KEY" -d '{"model":"Qwen/Qwen2.5-0.5B-Instruct","messages":[{"role":"user","content":"Say hi."}],"max_tokens":16}' 2>/dev/null)
